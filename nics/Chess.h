...
public:
  ...
  enum Result { Aborted, InProgress, Resign, Checkmate, Stalemate, TimeForfeit };
  ...
  virtual void make_move(SessionID player,
			 const string &move);
  virtual const string get_board(SessionID player,
				 Relationship rel, int style = 0 );
  virtual void refresh( SessionID player,
			Relationship rel ) const {};
  virtual void flag( SessionID player );
  virtual void resign( SessionID player );
