...
void Chess::make_move(NICSSessionManager::SessionID player,
...
// ICS style 12
const string Chess::get_board( NICSSessionManager::SessionID session,
  ...
  out << clocks[white].seconds() << ' '
      << clocks[black].seconds() << ' ';
...
void Chess::start() {
  ...
  game.white(UserSession::name(white));
  game.black(UserSession::name(black));
...
void Chess::flag( NICSSessionManager::SessionID player ) {
  Game::refresh();
...
void Chess::resign( NICSSessionManager::SessionID player ) {
  if ( player == white ) {
    winner = black;
  } else {
    winner = white;
