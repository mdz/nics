...
void PostgreSQL::get_user( int uid, User &user ) throw(UserNotFound) {
  ...
  query << "SELECT * FROM users WHERE uid = " << uid << ends;

  lock();
  if ( database.Exec(query.str()) != PGRES_TUPLES_OK) {
...
void PostgreSQL::save_game( const Chess &game ) {
  return;
...
void PostgreSQL::dump_history( const User &user, ostream &out,
  ...
  for( unsigned int i = 0 ; i < howmany && i < database.Tuples() ; ++i ) {
...
string PostgreSQL::last_opponent( const User &user ) {
  ostrstream query;
