...
NICSSessionManager::SessionID NICSSessionManager::next_session_id() const
...
NICSSessionManager::SessionID
NICSSessionManager::register_session(UserSession::pointer session)
  throw(SessionLimitReached) {
  ...
  try {
    id = next_session_id();
    //sessions[id] = element;
    sessions.insert( map<SessionID, UserSession::pointer>
		     ::value_type(id, session) );
...
void NICSSessionManager::unregister_session(SessionID session) throw() {
  ...
  map<SessionID, UserSession::pointer>::iterator it = sessions.find(session);
...
void NICSSessionManager::dump_session_info(ostream &out) throw() {
  ReadLock();
  for( map<SessionID, UserSession::pointer>::iterator it = sessions.begin() ;
       it != sessions.end() ; ++it )
    out << "Session ID " << it->first << ": "
	<< "User name: " << it->second->name() << endl;
  Unlock();
}

void NICSSessionManager::show_sessions(UserSession::pointer session)
  throw(UserNotFound) {

  ReadLock();
  for( map<SessionID, UserSession::pointer>::iterator it = sessions.begin() ;
       it != sessions.end() ; ++it )
    session->send_session(it->second);
  Unlock();
}
...
unsigned int NICSSessionManager::send_offer( GameOffer::pointer offer )
  throw(UserNotFound) {
  ReadLock();
  ...
  if ( offer->user_session_filter().session_specific() ) {
    ...
    map<SessionID, UserSession::pointer>::iterator it =
