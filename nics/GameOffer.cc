...
GameOffer::GameOffer( SessionID session,
  ...
  //  my_id = NICSCore::gm->register_offer( this );

}

GameOffer::~GameOffer() {
  //  NICSCore::gm->unregister_offer( my_id );
}

bool GameOffer::match( SessionID session ) const {
  if ( session == my_session )
    return false;

  try {
    bool ret = UserSession::rlock(session).match(my_game_filter);
